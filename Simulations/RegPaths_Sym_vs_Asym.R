...
fusedTree(Tree = Treefit, Y = Y, X = X, Z = Z,
          lambda = 10, alpha = 10, symFusion = F,
          model = "linear", LinVars = FALSE)$Effects[5:8]
fusedTree(Tree = Treefit, Y = Y, X = X, Z = Z,
          lambda = 10, alpha = 10, symFusion = T,
          model = "linear", LinVars = FALSE)$Effects[5:8]
library(tidyverse)
...
for (i in seq_along(lambda)) {
  ...
  df_sym <- do.call(rbind, Betas_sym) %>%
    ...
    mutate(alpha = alpha,
           lambda = Lam,
           symFusion = "Symmetric") %>%
    pivot_longer(cols = everything()[1:4],  # V1 to V4
  ...
  df_asym <- do.call(rbind, Betas_asym) %>%
    ...
    pivot_longer(cols = everything()[1:4],
                 names_to = "effect_index",
                 values_to = "effect_value")
...
getwd()
library(Cairo); library(ggplot2); library(viridis)
name <- paste("RegPath","Sym_vs_Asym", sep = "_")
CairoPDF(name, width=7,height=6)
ggplot(results_df, aes(x = alpha, y = effect_value, color = effect_index)) +
  geom_line(linewidth = 1.1) + scale_x_log10() +
  facet_grid(rows = vars(lambda_lab), cols = vars(symFusion)) +
  scale_color_viridis(discrete = T, option = "D", labels = effect_labels) +
  labs(title = "",

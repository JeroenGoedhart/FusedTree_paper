...
results1 <- results[,c(7,2,1,8,3,4,6)]
colnames(results1)<- c("Oracle","FusTree","FullFus","ZeroFus","GB","RF","Lasso")
...
pdf(name, width=7,height=7.5)
bp <- ggplot(dat, aes(x=ind, y=values, group=ind)) +
  geom_boxplot(aes(fill=ind),outlier.shape = NA,fatten=2) + coord_cartesian(ylim =  c(0, 45)) +
